...
// Return one of the E_* error codes
{
  ...
  _trackCount = dat16;
  Serial.printf("begin: found %d tracks\r\n",dat16);
  ...
  Serial.printf("From the header got %d _ticksPerQuarterNote\r\n",_ticksPerQuarterNote);
  ...
  for (uint8_t i = 0; i<_trackCount; i++)
  {
    ...
    //GG if ((err = _track[i].load(i, this)) != -1)
    err = _track[i].load(i, this);
    if ( err == 1)
    {
      Serial.printf("Cannot find the next track after track %d \r\n", i);  
    }else if(err == 0){
      Serial.printf("Header error for track %d \r\n", i);  
      return (10*(i+1))+err;   
    }
  }
  return E_OK;
